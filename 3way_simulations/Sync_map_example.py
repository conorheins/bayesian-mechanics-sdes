...
if save_mode:
    ...
    figures_folder = os.path.join(figures_folder, 'sync_map_example')
    if not os.path.isdir(figures_folder):
        os.mkdir(figures_folder)
...
sync = Pi[de, de] **(-1) * Pi[de,db] * Pi[di,db]**(-1) * Pi[di, di] # sync map (defined in terms of precisions)
#sync = S[de, db] * S[di, db] ** (-1)  # sync map (defined using stationary covariance)


'''
Generate samples of Gaussian distribution
'''
N = 10 ** 6   # number of parallel realizations #usually 3*10**2
...
if save_mode:
    figure_name = "Prediction_sync_map_example.png"
