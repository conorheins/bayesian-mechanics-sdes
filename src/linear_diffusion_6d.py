...
# initialization_key = 5    # default configuration in `config_6d.py` file if no key is passed, lots of solenoidal flow / oscillations
# initialization_key = 4  # good one, some solenoidal flow but not too much
# initialization_key = 3  # decent one, sigma has negative determinant
...
if save_mode:
    figure_name = f'Sample_perturbed_6wayOU_s{initialization_key}.png'
    plt.savefig(os.path.join(figures_folder,figure_name),dpi=100)
    plt.close()

'''
Figure 2: average time-varying F(b, bold mu), averaged over trajectories
'''
...
if save_mode:
    figure_name = f'FE_vs_time_perturbed_6wayOU_s{initialization_key}.png'
    plt.savefig(os.path.join(figures_folder,figure_name),dpi=100)
    plt.close()
...
over time, locked to beginning of the blanket-state perturbation. SINGLE PATH VARIANT 1
'''
...
if save_mode:
    figure_name = f'singlepath_prediction_blanket_perturbation_s{initialization_key}_bpath.png'
    plt.savefig(os.path.join(figures_folder,figure_name), dpi=100)
    plt.close()
...
over time, locked to beginning of the blanket-state perturbation. SINGLE PATH VARIANT 2
'''
...
if save_mode:
    figure_name = f'singlepath_prediction_blanket_perturbation_s{initialization_key}_mupath.png'
    plt.savefig(os.path.join(figures_folder,figure_name), dpi=100)
    plt.close()
...
over time, locked to beginning of the blanket-state perturbation. ENSEMBLE AVERAGE VARIANT. Average internal states and map through sigma(<mu_t>), with
average taken across realisations.
'''
...
if save_mode:
    figure_name = f'average_prediction_blanket_perturbation_s{initialization_key}.png'
    plt.savefig(os.path.join(figures_folder,figure_name), dpi=100)
    plt.close()
...
PARAMETERISED BY SIGMA(BOLDMU(B_T))
'''
...
plt.title('Precision-weighted prediction errors $\mathbf{\Pi}_{\eta\eta} (\eta - \mathbf{\mu})$',fontsize=26, pad = 10)
...
if save_mode:
    figure_name = f'ppe_cov_plot_boldmu_b_t_s{initialization_key}.png'
    plt.savefig(os.path.join(figures_folder,figure_name),dpi=100)
    plt.close()
...
if save_mode:
    figure_name = f'ppe_cov_plot_sigma_mu_t_s{initialization_key}.png'
    plt.savefig(os.path.join(figures_folder,figure_name),dpi=100)
    plt.close()
