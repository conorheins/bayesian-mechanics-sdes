'''
Simulations synchronisation map 6D (non-example)
'''
...
#Make sure sync map is not well defined.
Pi[di,2] = 0
Pi[2, di] =0
# enforce symmetric
Pi = (Pi + Pi.T) / 2
...
if rank(np.append(ker(Pi[di, db]), ker(Pi[de, db]), axis=1)) > rank(ker(Pi[de, db])):
    print("Synchronisation map not well defined")
...
for i in range(0, 2):
    ...
    plt.savefig(f"syncmap_6way_dim{i + 1}_noneg.png")
